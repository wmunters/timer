    ...
    def SessionTimer(self,master,headerlabel, label):
        for pres in self.presentations:
            ...
            while timeLeft>=0:
                ...
                if not master.Pause:
                    timeLeft = timeLeft-1
                elif master.OverRideTime: # Pause and override flag true
                    timeLeft = master.setTime
                    master.OverRideTime = False
                    master.Pause = False
            ...
            if not ('Break' in self.name or 'break' in self.name or master.Next):
                ...
                if not timeLeftq == 0:
                    while timeLeftq>=0:
                        ...
                        minutesLeft, secondsLeft = (timeLeftq // 60, timeLeftq % 60)
                        stringtime= "%02d:%02d" % (minutesLeft, secondsLeft)
                ...
                if not master.Next:
                    ...
                    while timeSwitch>=0:
                        ...
                        minutesLeft, secondsLeft = (timeSwitch // 60, timeSwitch % 60)
                        stringtime= "%02d:%02d" % (minutesLeft, secondsLeft)
